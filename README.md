...
**Black screen in dark mode.** With the system in night mode the app could launch to a completely
black screen — no text, no backgrounds, only a couple of icons — while networking worked fine and
nothing crashed. `ActionBarLayout.animateThemedValues` calls `Theme.setAnimatingColor(true)` and
seeds every animated key with its *start* colour; while that flag is set, `Theme.getColor` returns
those seeds. The flag is released only by the animator's end/cancel handlers, both guarded by "this
animator is still the current one", so a superseded or never-advancing animator leaks it for the
rest of the process and every colour in the app resolves to fully transparent. (The surviving icons
are `ImageView`s tinted with `setColorFilter`, because PorterDuff `SRC_ATOP` with a zero-alpha
source is a no-op and the drawable's own artwork shows through — which is what made the diagnosis
possible.) Fixed two ways: a fully transparent animated value no longer wins over the real colour,
and the flag has a watchdog.
